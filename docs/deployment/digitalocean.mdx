...
<HeaderBadgesWidget commaDelimitedContributors="JeysonFlores" lastVerifiedDateString="February 15th, 2023" />

[Digital Ocean's App Platform](https://docs.digitalocean.com/products/app-platform/) is a Platform-as-a-Service (PaaS) offering that allows developers to publish code directly to DigitalOcean servers without worrying about the underlying infrastructure.

App Platform does not support Rust in their set of available runtimes, however it can run any application packaged inside a Docker image.
...
CMD ["./myproject"]
```
> Remember that `myproject` is the name of the project. You may change it to match your own.
